...
object DocWordDistance {
  ...
  def main(args: Array[String]) {
    ...
    val wordInputFile = args(0)
    val docInputFile = args(1)
    val docId = args(2)
    loadWords(wordInputFile)
    loadDocs(docInputFile)
    play(docId)
  }
  ...
  def loadWords(embeddingsFile: String): Unit = {
    ...
    println("loaded word vocab and their embeddings")
  }

  def loadDocs(embeddingsFile: String): Unit = {
    var lineItr = Source.fromFile(embeddingsFile).getLines
    docNum=  lineItr.next.stripLineEnd.toInt
    println("DocNum "+docNum)
    println("Dimension "+D)
    docWeights = new Array[DenseTensor1](docNum)
    docs = new Array[String](docNum)
    for(d <- 0 until docNum){
      val line = lineItr.next.stripLineEnd.split(' ')
      docs(d)= line(0)
      docWeights(d) =  new DenseTensor1(D, 0)
      for (i <- 0 until D) docWeights(d)(i) = line(i + 1).toDouble
      docWeights(d)/= docWeights(d).twoNorm

    }
    println("loaded doc vocab and their embeddings")

  }
  def play(docId:String): Unit = {

    //while (true) {
      print("Enter docId (EXIT to break) : ")
      //var words = readLine.stripLineEnd.toLowerCase.split(' ').map(word => getID(word)).filter(id => id != -1)
      //var docid =readLine.stripLineEnd.map(did => getdocID(did)).filter(id => id != -1)
      var docid = getdocID(docId)
      println("DocId "+docid)

      if (docid == -1) {
        println("docId not in vocab")
      } else {
        val doc_embedding_in = new DenseTensor1(D, 0)
        //words.foreach(word => embedding_in.+=(weights(word)))
        doc_embedding_in.+=(docWeights(docid))
        ...
        for (i <- 0 until vocab.size){
          ...
          val score = TensorUtils.cosineDistance(doc_embedding_in, embedding_out)
